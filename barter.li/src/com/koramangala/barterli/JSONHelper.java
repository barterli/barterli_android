...
public class JSONHelper {
	...
	public String[] getBookNamesFromUserProfile(String UserJSONString){
		...
		try {
			...
			if(!userObject.has("books") || userObject.getJSONArray("books").length()==0){
				return new String[0];
			}
			JSONArray booksArray = userObject.getJSONArray("books");
			final String[] bookTitleArray = new String[booksArray.length()];
			for (int i = 0; i < booksArray.length(); i++){	
				bookTitleArray[i] = booksArray.getJSONObject(i).getString("title");	
			} 
	...
	public JSONArray getBookObjectsFromUserProfile(String UserJSONString){
		...
		try {
			...
			for (int i = 0; i < booksArray.length(); i++){	
				...
				_obj.put("description", booksArray.getJSONObject(i).opt("description"));
				_obj.put("publication_year", booksArray.getJSONObject(i).opt("publication_year"));
				_obj.put("barter_type", booksArray.getJSONObject(i).opt("barter_type"));
