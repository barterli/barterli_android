...
public final class Crouton {
    ...
    public static Crouton makeText(final Activity activity,
                    final int textResourceId, final Style style,
                    final int viewGroupResId) {
        return makeText(activity, activity.getString(textResourceId), style, (ViewGroup) activity
                        .findViewById(viewGroupResId));
    }
    ...
    public Animation getInAnimation() {
        if ((null == this.inAnimation) && (null != this.activity)) {
            if (getConfiguration().inAnimationResId > 0) {
                this.inAnimation = AnimationUtils
                                .loadAnimation(getActivity(), getConfiguration().inAnimationResId);
            } else {
                measureCroutonView();
    ...
    public Animation getOutAnimation() {
        if ((null == this.outAnimation) && (null != this.activity)) {
            if (getConfiguration().outAnimationResId > 0) {
                this.outAnimation = AnimationUtils
                                .loadAnimation(getActivity(), getConfiguration().outAnimationResId);
            } else {
    ...
    private void measureCroutonView() {
        ...
        int widthSpec;
        if (viewGroup != null) {
            widthSpec = View.MeasureSpec.makeMeasureSpec(viewGroup
                            .getMeasuredWidth(), View.MeasureSpec.AT_MOST);
        } else {
            widthSpec = View.MeasureSpec
                            .makeMeasureSpec(activity.getWindow()
                                            .getDecorView().getMeasuredWidth(), View.MeasureSpec.AT_MOST);
        }

        view.measure(widthSpec, View.MeasureSpec
    ...
    private FrameLayout initializeCroutonViewGroup(final Resources resources) {
        final FrameLayout croutonView = new FrameLayout(this.activity);
        ...
        if (this.style.backgroundDrawableResourceId != 0) {
            final Bitmap background = BitmapFactory
                            .decodeResource(resources, this.style.backgroundDrawableResourceId);
    ...
    private RelativeLayout initializeContentView(final Resources resources) {
        final RelativeLayout contentView = new RelativeLayout(this.activity);
        ...
        final RelativeLayout.LayoutParams textParams = new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
        if (null != image) {
    ...
    private void initializeTextViewShadow(final Resources resources,
                    final TextView text) {
        final int textShadowColor = resources
                        .getColor(this.style.textShadowColorResId);
        final float textShadowRadius = this.style.textShadowRadius;
        final float textShadowDx = this.style.textShadowDx;
        final float textShadowDy = this.style.textShadowDy;
    ...
    private ImageView initializeImageView() {
        ...
        final RelativeLayout.LayoutParams imageParams = new RelativeLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
